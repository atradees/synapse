...
const Scene = ({ 
  ...
  // --- 1. DYNAMIC MATERIALS (FIX ERROR EMISSIVE) ---
  const materials = useMemo(() => {
    const { up, down, base } = theme.colors;
    return {
      // MeshStandardMaterial support emissive
      ...
      // MeshBasicMaterial (Transparent) - NO EMISSIVE HERE
      neutral: new THREE.MeshBasicMaterial({ color: base, transparent: true, opacity: 0.3 }),
      // FIX: Ganti ke Standard agar support emissive glowing
      locked: new THREE.MeshStandardMaterial({ color: "#ffffff", emissive: "#ffffff", emissiveIntensity: 2, roughness: 0.0 }), 
    };
  }, [theme]);

  // --- 2. NEIGHBOR CALCULATION (FOR ISOLATION) ---
  ...
  // --- 3. LIGHTING FIX (MANUAL ADDITION - NO JSX WARNINGS) ---
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (controlMode === 'fly') {
        ...
        const onLock = () => { setIsLocked(true); velocity.current.set(0, 0, 0); };
        const onUnlock = () => {
            setIsLocked(false);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (controlsRef.current && controlsRef.current.isLocked) {
        ...
        if (fgRef.current.controls()) fgRef.current.controls().enabled = false;
        ...
        if (data && data.nodes) {
            for (let node of data.nodes) {
                ...
                if (focusTarget) {
                    ...
                    const isTarget = node.id === focusTarget.id;
                    if (!isTarget && !isNeighbor) continue;
                }
                ...
                if (dot > minDotThreshold && dot > bestDot) { bestDot = dot; bestTarget = node; }
            }
        }
        if (aimedNodeRef.current !== bestTarget) { aimedNodeRef.current = bestTarget; setAimedNode(bestTarget); }
        if (bestTarget) {
            const dist = Math.sqrt(Math.pow(bestTarget.x - camera.position.x, 2) + Math.pow(bestTarget.y - camera.position.y, 2) + Math.pow(bestTarget.z - camera.position.z, 2));
  ...
  }, [controlMode, data, focusTarget, neighbors]);

  // PULSE & BLOOM
  useEffect(() => {
    let frameId;
    const animate = () => { setPulse(Math.sin(Date.now() / 1000 * 2)); frameId = requestAnimationFrame(animate); };
    animate();
    return () => cancelAnimationFrame(frameId);
  }, []);
  ...
  return (
    <div className="w-full h-full relative group">
      <ForceGraph3D
        ...
        onNodeClick={(node) => { if (controlMode !== 'fly') onNodeClick(node); }}
        ...
        nodeThreeObject={node => {
          ...
          if (change > 0) mat = materials.up;
          else if (change < 0) mat = materials.down;
        ...
        linkVisibility={link => {
            ...
            if (focusTarget) {
               return src === focusTarget.id || tgt === focusTarget.id;
            }
            return true; 
        }}
        
        linkWidth={link => Math.abs(link.value) * 1.5}
        linkColor={link => link.value < 0 ? theme.colors.down : theme.colors.up}
      ...
      {/* HUD & PILOT OVERLAY (Sama seperti sebelumnya) */}
      {controlMode === 'fly' && !isLocked && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/60 cursor-pointer z-50 backdrop-blur-sm" onClick={() => controlsRef.current?.lock()}>
            <div className="text-center border p-8 rounded-2xl bg-black/80 shadow-2xl animate-pulse" style={{ borderColor: `${theme.colors.up}40` }}>
      ...
      {controlMode === 'fly' && isLocked && (
        <>
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none transition-all duration-75 ${aimedNode ? 'scale-125' : 'scale-100'}`}>
                <div className="w-8 h-8 border border-white/40 rounded-sm flex items-center justify-center" style={{ borderColor: aimedNode ? theme.colors.up : 'rgba(255,255,255,0.4)' }}>
                    <div className="w-1 h-1 bg-white shadow-lg" style={{ backgroundColor: aimedNode ? theme.colors.up : 'white', boxShadow: aimedNode ? `0 0 10px ${theme.colors.up}` : 'none' }} />
                </div>
            </div>
            {aimedNode && (
                <div className="absolute top-[53%] left-[53%] pointer-events-none flex flex-col gap-2">
                    <div className="bg-black/90 p-3 backdrop-blur-xl shadow-2xl min-w-[240px] animate-in fade-in zoom-in-95 duration-100" style={{ borderTop: `2px solid ${theme.colors.up}` }}>
                        <div className="flex justify-between items-start mb-2">
                            <div><div className="text-[9px] font-mono tracking-widest mb-0.5" style={{ color: theme.colors.up }}>TARGET_LOCKED</div><div className="text-lg font-black text-white leading-none">{aimedNode.ticker || aimedNode.id}</div></div>
                            <div className="text-right"><div className="text-[9px] text-white/40 font-mono">DIST</div><div className="text-xs text-white font-mono">{aimDistance}m</div></div>
                        </div>
                        <div className="mb-3"><div className="text-[10px] text-white/80 font-bold truncate">{aimedNode.name}</div><div className="text-[9px] text-white/40 font-mono">{aimedNode.group}</div></div>
                        <div className="border-t border-white/10 pt-2 grid grid-cols-2 gap-2 text-[9px] font-mono mb-2">
                             <div><div className="text-white/40">{timeframe} CHG</div><div style={{ color: aimedNode.periodChange >= 0 ? theme.colors.up : theme.colors.down }}>{aimedNode.periodChange}%</div></div>
                             <div><div className="text-white/40">Z-SCORE</div><div style={{ color: aimedNode.zScore > 0 ? theme.colors.up : theme.colors.down }}>{aimedNode.zScore}</div></div>
                        </div>
                    </div>
                </div>
            )}
